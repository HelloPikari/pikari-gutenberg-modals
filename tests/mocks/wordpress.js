...
const wp = {
	i18n: {
		...
		_n: jest.fn( ( single, plural, count ) => ( count === 1 ? single : plural ) ),
	...
	richText: {
		...
		applyFormat: jest.fn( ( value, format ) => ( { ...value, activeFormats: [ format ] } ) ),
		removeFormat: jest.fn( ( value ) => ( { ...value, activeFormats: [] } ) ),
	...
	hooks: {
		...
		applyFilters: jest.fn( ( hook, value, ...args ) => {
			...
			return hooks.filters
				...
				.reduce( ( val, filter ) => filter.callback( val, ...args ), value );
		} ),
	},
};
