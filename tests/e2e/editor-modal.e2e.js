...
test.describe( 'Modal Link Editor Experience', () => {
	...
	test( 'should add modal link to paragraph', async ( { page } ) => {
		...
		// Select first result
		await page.click(
			'.block-editor-link-control__search-item:first-child'
		);
	...
	test( 'should edit existing modal link', async ( { page } ) => {
		...
		await page.evaluate( () => {
			const block = wp.blocks.createBlock( 'core/paragraph', {
				content:
					'Text with <span class="modal-link-trigger" data-modal-content-type="post" data-modal-content-id="1">existing modal</span>.',
			} );
		...
		await page.click(
			'button[aria-label="Modal Link"][aria-pressed="true"]'
		);
	...
	test( 'should remove modal link', async ( { page } ) => {
		...
		await page.evaluate( () => {
			const block = wp.blocks.createBlock( 'core/paragraph', {
				content:
					'Text with <span class="modal-link-trigger" data-modal-content-type="post" data-modal-content-id="1">modal to remove</span>.',
			} );
			wp.data.dispatch( 'core/block-editor' ).insertBlock( block );
		} );
	...
	test( 'should handle external URL modal', async ( { page } ) => {
		...
		expect( content ).toContain(
			'data-modal-content-id="https://example.com"'
		);
	} );

	test( 'should not show modal button for unsupported blocks', async ( {
		page,
	} ) => {
		// Add a button block
		await page.click( 'button[aria-label="Add block"]' );
