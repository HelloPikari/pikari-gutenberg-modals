...
test.describe( 'Frontend Modal Display', () => {
	...
	test( 'should load external URL content', async ( { page } ) => {
		...
		const externalTrigger = await page
			.locator( '.modal-link-trigger[data-modal-content-type="url"]' )
			.first();
	...
	test( 'should handle keyboard navigation for triggers', async ( {
		page,
	} ) => {
		...
		const focusedTrigger = await page.locator(
			'.modal-link-trigger:focus'
		);
		...
		if ( ( await focusedTrigger.count() ) > 0 ) {
			...
			// Verify modal opened
			await expect( page.locator( '.modal-overlay' ) ).toHaveClass(
				'is-open'
			);
