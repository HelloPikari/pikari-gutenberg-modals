...
describe( 'ModalLinkEdit Component Behavior', () => {
	...
	describe( 'Modal Data Handling', () => {
		it( 'should parse modal link data from attributes', () => {
			...
			let modalData;
			try {
				modalData = JSON.parse( mockFormat.attributes[ 'data-modal-link' ] );
			} catch ( error ) {
				modalData = null;
			}
		...
		it( 'should handle legacy format data', () => {
			...
			const modalData = {
				...
				type: mockFormat.attributes[ 'data-modal-content-type' ] || 'post',
			};
	...
	describe( 'Format Application', () => {
		it( 'should create format object for internal links', () => {
			...
			expect( format.attributes[ 'data-modal-content-type' ] ).toBe( 'page' );
			expect( format.attributes[ 'data-modal-content-id' ] ).toBe( '789' );
		} );
		...
		it( 'should create format object for external URLs', () => {
			...
			expect( format.attributes[ 'data-modal-content-type' ] ).toBe( 'url' );
			expect( format.attributes[ 'data-modal-content-id' ] ).toBe( 'https://example.com' );
		} );
	} );
} );