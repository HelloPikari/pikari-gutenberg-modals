...
const { wp, getRegisteredFormat, clearAllMocks } = require( '../mocks/wordpress' );
...
describe( 'Modal Format Registration', () => {
	...
	it( 'should have required attributes', () => {
		...
		expect( attributes[ 'data-modal-content-type' ] ).toBe( 'data-modal-content-type' );

		expect( attributes ).toHaveProperty( 'data-modal-content-id' );
		expect( attributes[ 'data-modal-content-id' ] ).toBe( 'data-modal-content-id' );
	} );
