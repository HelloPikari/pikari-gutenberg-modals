...
- Server-cached for logged-out visitors: the response data (before `pikari_gutenberg_modals_content_response`, which still runs on every request) is stored in a transient keyed by the ETag, for the cache duration, so a hit skips the render and the simulated `wp_enqueue_scripts`/`wp_footer`. Measured in wp-env: a miss fires both actions, a hit neither. The ETag hashes the post, not what it pulls in, so a changed template part or Query Loop result shows when the transient expires; a browser revalidating against that same ETag gets a 304 and keeps its copy until the post itself changes. `RestApi::should_share_render()` gates both read and write: logged out, no query parameter but `modal_id`/`rest_route`, no `comment_author_*` or `wp-postpass_*` cookie — a review found `?query-0-page=999` would otherwise fix a blank Query Loop for every visitor. It assumes a render depends only on the post; a site varying by device, language cookie or session should filter the duration to 0. Storage is one option row per public post fetched per ETag, left for core's daily expired-transient sweep. Duration 0 skips both reading and storing (a transient stored with 0 would never expire)
...
11. **Block support CSS in wp_footer** — `BlockSupport::render_modal_containers()` snapshots block support CSS before rendering template parts and outputs any newly generated layout/spacing CSS in a `<style>` tag (needed because `wp_enqueue_block_support_styles()` runs earlier). It does the same for inline CSS appended to stylesheets that have already printed (`BlockStyleCollector::snapshot_printed_inline_styles()` / `collect_inline_styles_added_since()`, output as `modal-late-inline-styles`). Block style variations are the measured case: core appends each instance's `is-style-{variation}--N` rule to `block-style-variation-styles` on `render_block_data`, a block theme prints that handle in `<head>` because the template renders before `wp_head()`, and core never prints a done handle again — so a template part's variation lost its styling on the frontend while the editor looked right
...
pikari_gutenberg_modals_cache_duration         // How long a logged-out response is reused: HTTP max-age and the server-side render cache (default: HOUR_IN_SECONDS; 0 turns both off); a logged-in viewer's response is never stored
